 ...
 *
 * <p>Links a {@link UserEntity} to a {@link BoulderEntity} and enforces that each boulder can only
 * be marked once per user. The association is cascaded on delete so boulder removal by setters does
 * not fail because of project references.
 */
@Entity
@Table(
    name = "projects",
    uniqueConstraints =
        @UniqueConstraint(
            name = "uk_user_boulder_project",
            columnNames = {"user_id", "boulder_id"}))
