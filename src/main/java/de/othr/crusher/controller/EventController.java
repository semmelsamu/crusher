...
public class EventController {
  ...
  public String showEditForm(
      @PathVariable("gymId") long gymId, @PathVariable("eventId") long eventId, Model model) {
  ...
  /**
   * Handles the creation of a new event. Validates input and either redisplays the form with errors
   * or saves the new event.
   *
   * @param gymId identifier of the parent gym
      ...
      for (UserEntity user : users) {
        try {
          emailService.sendNewEventEmail(
              user.getEmail(), user.getName(), gym.getName(), savedEvent);
        } catch (Exception e) {
          System.err.println(
              "Failed to send event email to " + user.getEmail() + ": " + e.getMessage());
        }
      }
    }
  ...
  /**
   * Updates an existing event. Validates input and either redisplays the form with errors or saves
   * the changes.
   *
   * @param gymId identifier of the parent gym
   * @param eventId identifier of the event
  ...
  private EventEntity findEventInGymOrThrow(long gymId, long eventId) {
    ...
    if (event.getGym() == null
        || event.getGym().getId() == null
