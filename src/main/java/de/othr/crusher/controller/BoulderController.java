 ...
 *
 * <p>Provides basic CRUD operations for boulder problems, including color and grade assignment.
 */
@Controller
...
public class BoulderController {
  ...
  /**
   * Creates a new boulder for the given sector. Validates input and either redisplays the form with
   * errors or saves the new boulder.
   *
   * @param gymId identifier of the parent gym
   * @param sectorId identifier of the parent sector
  ...
  public String createBoulder(
    ...
    if (boulder.getGrade() != null) {
      GradeEntity grade =
          gradeRepository
              .findById(boulder.getGrade().getId())
              .orElseThrow(
                  () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid grade"));
      if (!grade.getGym().getId().equals(gymId)) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Grade does not belong to this gym");
      }
  ...
  public String updateBoulder(
    ...
    if (formBoulder.getGrade() != null) {
      GradeEntity grade =
          gradeRepository
              .findById(formBoulder.getGrade().getId())
              .orElseThrow(
                  () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid grade"));
      if (!grade.getGym().getId().equals(gymId)) {
        throw new ResponseStatusException(
  ...
  private BoulderEntity findBoulderInSectorOrThrow(long sectorId, long boulderId) {
    ...
    if (boulder.getSector() == null
        || boulder.getSector().getId() == null
